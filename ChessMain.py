...
class Main():
    ...
    def mainloop(self):
        ...
        # Giu lai vi tri hai o vuong ma nguoi choi chon de di nuoc co. VD: [(0, 7), (3, 1)] - chua 2 toa do
        playerClick = []
        ...
        while running:
            ...
            humanTurn = (self.gs.whiteToMove and player1) or (
                not self.gs.whiteToMove and player2)
                ...
                if e.type == pg.QUIT:
                    ...
                    if not gameOver and humanTurn:
                        ...
                        if sqSelected == (row, col) or row >= 8 or col >= 8:
                            ...
                            sqSelected = (row, col)
                            # Luu lai 2 toa do 2 o vuong da click
                            playerClick.append(sqSelected)
                        if len(playerClick) == 2:
                            move = ChessEngine.Move(
                                playerClick[0], playerClick[1], self.gs.board)
            ...
            self.board.drawGameState(
                self.screen, self.gs, validMoves, sqSelected)

            if self.gs.checkMate:
                gameOver = True
                if self.gs.whiteToMove:
                    self.board.drawText(
                        self.screen, 'QUAN DEN THANG BOI CHIEU TUONG', "Gray", "Black")
                else:
                    self.board.drawText(
                        self.screen, 'QUAN TRANG THANG BOI CHIEU TUONG', "Black", "White")
